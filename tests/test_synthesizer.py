...
from staite.synthesizer import (
    SynthesisCache,
    SynthesisRecord,
    _should_regenerate,
    synthesize,
...
class TestShouldRegenerate:
    def test_no_cache_always_regenerates(self):
        assert _should_regenerate(0, 10, 0.2, has_cached=False) is True

    def test_below_threshold_uses_cache(self):
        assert _should_regenerate(1, 10, 0.2, has_cached=True) is False  # 10% < 20%

    def test_at_threshold_regenerates(self):
        assert _should_regenerate(2, 10, 0.2, has_cached=True) is True  # 20% == 20%

    def test_above_threshold_regenerates(self):
        assert _should_regenerate(5, 10, 0.2, has_cached=True) is True  # 50% > 20%

    def test_zero_total_files_no_regen(self):
        assert _should_regenerate(0, 0, 0.2, has_cached=True) is False

    def test_all_files_changed_regenerates(self):
        assert _should_regenerate(10, 10, 0.2, has_cached=True) is True
...
class TestSynthesisCache:
    ...
    def test_load_existing_file(self, tmp_path: Path):
        ...
        assert cache.record.use_cases == "UC1"
    ...
    def test_reload_after_save(self, tmp_path: Path):
        ...
        assert reloaded.record.use_cases == "scenarios"
...
class TestSynthesize:
    ...
    async def test_regenerates_when_no_cache(self, tmp_path: Path):
        ...
        assert result.use_cases == "Use cases here."
    ...
    async def test_uses_cache_below_threshold(self, tmp_path: Path):
        ...
        assert result.use_cases == "Cached UC"
    ...
    async def test_regenerates_above_threshold(self, tmp_path: Path):
        ...
        assert result.regenerated is True
        assert result.use_cases == "New UC"
