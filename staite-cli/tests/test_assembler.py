...
SAMPLE_DESCRIPTIONS = {
    "src/cli.py": "Entry point of the application.",
    "src/utils.py": "Utility helpers shared across modules.",
}

SAMPLE_TREE = ["myproject/", "├── src/", "│   ├── cli.py", "│   └── utils.py"]
...
class TestAssemble:
    ...
    def test_contains_file_tree(self):
        ...
        assert "cli.py" in doc["file_tree"]
    ...
    def test_contains_all_file_paths(self):
        doc = _parse(_assemble())
        assert "src/cli.py" in doc["files"]
        assert "src/utils.py" in doc["files"]

    def test_contains_file_descriptions(self):
        doc = _parse(_assemble())
        assert doc["files"]["src/cli.py"] == "Entry point of the application."
