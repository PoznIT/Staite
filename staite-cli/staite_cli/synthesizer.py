...
def _use_cases_prompt(
    ...
    return (
        ...
        + "Generate a Mermaid use case diagram for this project.\n"
          "Rules:\n"
          "- Use only actors and use-cases directly evidenced by the file descriptions.\n"
...
async def synthesize(
    ...
    should_regen = force_regen or should_regenerate(
    ...
    if not should_regen:
        ...
        return SynthesisResult(
            use_cases_diagram=cache.record.use_cases,
    ...
    return SynthesisResult(
        use_cases_diagram=use_cases_msg,
...
async def _call_both(
    ...
    use_cases, conventions = await asyncio.gather(
        provider.complete(_use_cases_prompt(project_name, tree_str, descs_str, instructions), 2048),
        provider.complete(_conventions_prompt(project_name, tree_str, descs_str, user_conventions), 2048),
    )
